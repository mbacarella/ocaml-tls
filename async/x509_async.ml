...
module Authenticator = struct
  ...
  module Param = struct
    ...
    let ca_file ?allow_ca_cert ?allowed_hashes ?crls filename () =
    ...
    let ca_dir ?allowed_hashes ?crls directory_name () =
      ...
      Chain_of_trust { trust_anchors; allowed_hashes; crls; allow_ca_cert = None }
    ;;
    ...
    let of_cas ~time ({ trust_anchors; allowed_hashes; crls; allow_ca_cert } : Chain_of_trust.t) =
      ...
      let%map crls =
        ...
        | None -> return None
      in
      X509.Authenticator.chain_of_trust ?allow_ca_cert ?allowed_hashes ?crls ~time cas
    ;;
